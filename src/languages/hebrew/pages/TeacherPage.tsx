...
import { Classroom } from '@he/teacher/Classroom';
import { PlacementFlow } from '@he/teacher/PlacementExam';
...
function LessonRunner({
  ...
  const onScored = useCallback(
    (correct: boolean, hinted: boolean, elapsedMs: number) => {
      ...
      if (!src) return;
      void recordAnswer(scopedKey(dil.id, itemKey(src.kind, src.id, src.axis)), 'ogretmen', {
        correct,
        elapsedMs,
    ...
    [step.question, award, level, dil.id],
  );
...
export default function TeacherPage() {
  ...
  if (lessonId === 'seviye-tespit') {
    return <PlacementFlow onApplyLevel={setLevel} onExit={() => navigate(yol('ogretmen'))} />;
  }
